...
class EmergencyEventController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // $emergencyEvents = EmergencyEvent::all();
            $emergencyEvents = EmergencyEvent::orderBy('ee_id', 'DESC')->paginate('6'); 
        ...
        if ($request->ajax()) {
            foreach ($emergencyEvents as $result) {
                $img_url = 'images/'.$result->event_img;
                if(strpos($result->event_img, 'http') === 0) // if check img url is http or https
                    $img_url = $result->event_img;
                $data .=  '<div class="card-body shadow-sm">
                            <div class="card">
                                <a href="/'.$result->ee_id.'">
                                    <img class="card-img-top" src="'.$img_url.'" style="width:100%">
                                </a>
